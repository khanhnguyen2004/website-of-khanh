'use client';
import style from '@/app/cart/styles/app.cart.module.css';
import { useState } from 'react';
const AppCart = () => {
    const products = [
        { id: 1, name: 'Gradient Graphic T-shirt', size: 'Large', color: 'White', price: 145, img: '/img/image-8-2.png' },
        { id: 2, name: 'Ckeckered Shirt', size: 'Medium', color: 'Red', price: 180, img: '/img/image-9.png' },
        { id: 3, name: 'Skinny Fit Jeans', size: 'Large', color: 'Blue', price: 240, img: '/img/image-8.png' },
    ];
    const [amounts, setAmounts] = useState(products.map(() => 1));
    const updateAmount = (index: number, value: number) => {
        setAmounts(prev => prev.map((a, i) => (i === index ? Math.max(0, value) : a)));
    };
    return (
        <>
                    ...
                    {products.map((product, index) => (
                        <div key={product.id} className={style["item"]}>
                                ...
                                <div className={style["right"]}>
                                    <div className={style["info-product"]}>
                                        ...
                                        <div className={style["variant"]}>
                                            <p className={style["size"]}>
                                                <span className="">Size: </span> <span className={style["name-size"]}>{product.size}</span>
                                            </p>
                                            <p className={style["color"]}>
                                                <span className="">Color: </span> <span className={style["name-color"]}>{product.color}</span>
                                            </p>
                                        </div>
                                    ...
                                    <div className={style["control"]}>
                                        <div className={style["delete"]}>
                                            <a href={"#"}>
                                                <svg xmlns="http://www.w3.org/2000/svg" className={style["icon-trash"]} width="18"
                                        ...
                                        <div className={style["amount"]}>
                                            <div className={style["control-amount"]}>
                                                <button onClick={() => updateAmount(index, amounts[index] - 1)}>
                                                    <svg xmlns="http://www.w3.org/2000/svg" className={style["icon-minus"]} width="16"
                                                        height="2" viewBox="0 0 16 2" fill="none">
                                                        <path
                                            ...
                                            <input type="text" className={`${style["amount-input"]} ${style["smooth-change"]}`} value={amounts[index]} onChange={(e) => updateAmount(index, Number(e.target.value))} />
                                            <div className={style["control-amount"]}>
                                                <button onClick={() => updateAmount(index, amounts[index] + 1)}>
                                                    <svg xmlns="http://www.w3.org/2000/svg" className={style["icon-plus"]} width="16"
                            ...
                            {index < products.length - 1 && (
                                <svg xmlns="http://www.w3.org/2000/svg" className={style["line-4"]} width="1240" height="1"
                                    viewBox="0 0 1240 1" fill="none">
                                    <line x1="-4.37114e-08" y1="0.500122" x2="1240" y2="0.500014" stroke="black"
                                        strokeOpacity="0.1">
                                    </line>
                                </svg>
                            )}
