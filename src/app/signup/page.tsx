...
const SignUp = () => {
    ...
    const validationForm = () => {
        ...
        if (!usernameRegex.test(username)) {
            toast.error("Username must be 5–20 characters and contain only letters, numbers, or underscores.");
            return false;
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!username || !password || !email || !firstName || !lastName || !phoneNumber || !confirmPassword) {
                    ...
                    <div className={style["form-signup"]}>
                        ...
                        <form onSubmit={handleSubmit} method="POST">
                            <div className={style["input-group"]}>
                                ...
                                <input type="text" id="first-name" name="first-name" value={firstName} onChange={(e) => setFirstName(e.target.value)} placeholder="Enter your first-name" required />
                            </div>
                            <div className={style["input-group"]}>
                                <label htmlFor="last-name">Last name</label>
                                <input type="text" id="last-name" name="last-name" value={lastName} onChange={(e) => setLastName(e.target.value)} placeholder="Enter your last-name" required />
                            </div>
                            <div className={style["input-group"]}>
                            ...
                            <div className={style["input-group"]}>
                                ...
                                <input type="text" id="phone-number" name="phone-number" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} placeholder="Enter your phone-number" required />
                            </div>
                            <div className={style["input-group"]}>
                            ...
                            <div className={style["input-group"]}>
                                ...
                                <div className={style["input-wrapper"]}>
                                    <input type={showConfirm ? "text" : "password"} id="password" name="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Confirm your password" required />
                                    <span onClick={() => setShowConfirm(!showConfirm)} className={style["icon-show"]}>
                                        {showConfirm ?
                                            (<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                fill="#000000" viewBox="0 0 24 24" >
                                                <path d="M12 9a3 3 0 1 0 0 6 3 3 0 1 0 0-6"></path><path d="M12 19c7.63 0 9.93-6.62 9.95-6.68.07-.21.07-.43 0-.63-.02-.07-2.32-6.68-9.95-6.68s-9.93 6.61-9.95 6.67c-.07.21-.07.43 0 .63.02.07 2.32 6.68 9.95 6.68Zm0-12c5.35 0 7.42 3.85 7.93 5-.5 1.16-2.58 5-7.93 5s-7.42-3.84-7.93-5c.5-1.16 2.58-5 7.93-5"></path>
                            ...
                            <button type="submit" className={style["btn-signup"]}>Sign Up</button>
                        </form>
                    </div>
                </div>
            </div>
